...
(async () => {
  ...
  if (hasReady) {
    ...
    // for cases where HTML does not strictly follow the starter tick template
    // (e.g. lastTick uses performance.now() causing font loading time to be
    // counted in the first frame's dt).
    ...
    if (seekCorrected) {
      // Wait two rAF for seek to take effect and render t=0 frame
