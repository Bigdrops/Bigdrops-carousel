...
#
# Pipeline:
#   1. render-video.js  record silent MP4 (at timeline.totalDuration)
...
#   --timeline=<path>     timeline.json path (auto-reads totalDuration and voiceover.mp3 path)
#
# Optional:
...
#   --keep-silent         Keep intermediate artifact (silent MP4), for debugging
#   --seek                Use render-video-seek.js per-frame seek rendering (true 60fps · deterministic · no black frames)
...
if [ -z "$TIMELINE" ] || [ ! -f "$TIMELINE" ]; then
  echo "✗ Missing --timeline=<path> (timeline.json generated by narrate-pipeline.mjs)" >&2
  exit 1
fi
...
# Record duration = total duration + 1s safety buffer
...
echo ""
if [ -n "$USE_SEEK" ]; then
  echo "▸ Step 1/2 · Per-frame seek render HTML animation (silent · ${SEEK_FPS}fps deterministic)"
...
echo ""
echo "✓ Done: $OUT"
[ -n "$KEEP_SILENT" ] && echo "  (intermediate preserved: $SILENT_MP4)"
