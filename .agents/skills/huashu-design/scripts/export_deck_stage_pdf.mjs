 ...
 *   - In this case `export_deck_pdf.mjs` (multi-file specific) cannot be used
 *
 * Why can't we just `page.pdf()` (pitfall logged 2026-04-20):
 ...
 *
 * Solution:
 *   After opening the HTML, use page.evaluate to extract all section elements from the deck-stage slot,
 *   attach them under a plain div in body, with inline style forcing position:relative + fixed size,
 *   each section gets page-break-after: always, last one changed to auto to avoid trailing blank page.
 *
 * Dependencies: playwright
 *   npm install playwright
 *
 * Output features:
 *   - Text remains vector (selectable, searchable)
 *   - Visual 1:1 fidelity
...
async function main() {
  ...
  const sectionCount = await page.evaluate(({ W, H }) => {
    ...
    if (!stage) throw new Error('<deck-stage> not found — this script only works for single-file deck-stage architecture');
