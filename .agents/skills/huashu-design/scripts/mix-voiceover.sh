...
#
# Required:
#   --voiceover=<path>    Path to voiceover mp3 (main vocal track, from narrate-pipeline.mjs)
#
# Optional:
...
#   --no-ducking          Disable sidechain ducking (enabled by default: BGM auto-ducks when voice is active)
#   --voice-volume=<0-2>  Voice volume multiplier, default 1.0
#   --out=<path>          Output path, default <input>-voiced.mp4
#
# Behavior:
#   - Video stream copy (no re-encode, fast)
#   - Voice is always the main track, required; BGM is optional
#   - Ducking enabled by default: BGM drops to ~-10dB when voice is active, recovers during silence
#   - Output length = video length (shorter voice/BGM gets trailing silence; longer gets truncated)
#
# Examples:
