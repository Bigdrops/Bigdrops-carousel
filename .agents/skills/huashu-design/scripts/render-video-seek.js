 ...
 *
 * This is a per-frame alternative renderer to render-video.js (Playwright recordVideo).
 * The technical core borrows from HeyGen HyperFrames (Apache 2.0) — the idea of
 * "freezing the clock + seeking to timestamps for screenshots" — but does not
 * introduce any third-party packages. It only uses the existing playwright + ffmpeg
 * already in this skill, and is runtime-agnostic.
 *
 * Compared to render-video.js, this solves three deadlocks (see references/video-export.md § "seek rendering"):
 *   1. Frame rate is no longer locked to 25fps by Chromium headless compositor — --fps supports native arbitrary frame rates
 *   2. No longer needs convert-formats.sh's minterpolate post-hoc interpolation (which has ghosting + macOS
 *      QuickTime compatibility bug, see animation-pitfalls §14) — every frame is a real seek capture
 *   3. No screen recording → no leading black frames → no need for --trim / --fontwait / __ready offset logic
 *   Bonus: seeking to timestamps for screenshots, same input same output deterministic (recordVideo is real-time recording, non-deterministic)
 *
 * Prerequisite: animations must use the Stage clock (<Stage> from assets/animations.jsx
 * or <NarrationStage> from narration_stage.jsx), which respond to window.__seekRender
 * by freezing the self-driven clock and exposing window.__seek(t). Purely CSS @keyframes /
 * Lottie / non-Stage-driven animations do not support __seek; for those continue using render-video.js.
 *
...
const CONCURRENCY = Math.max(1, parseInt(arg('concurrency', '4')));  // parallel worker count (one page each)
const SETTLE      = Math.max(1, parseInt(arg('settle', '2')));       // number of rAF to wait after seek before screenshot
...
// Same chrome-hiding rules as render-video.js (ensures output consistency between both pipelines)
...
// Run in page context: wait SETTLE rAF (let React/Babel commit + layout stabilize before screenshot)
...
// A single worker: open a page, goto, wait for __seek readiness, render its assigned frames
async function renderFrames(context, url, frames) {
  const page = await context.newPage();
  await page.goto(url, { waitUntil: 'load', timeout: 60000 });

  // Stage / NarrationStage in __seekRender mode expose window.__seek and freeze the self-driven clock
...
(async () => {
  ...
  // Key signal: __seekRender tells Stage / NarrationStage to freeze the wall-clock rAF,
  // instead driving frames externally via __seek.
  // __recording is reused to force Stage loop=false (reusing existing convention).
  ...
  // Round-robin frames across CONCURRENCY workers (each page has its own window, seeks do not interfere)
  ...
  try {
    ...
    if (/__seek|__ready/.test(msg)) {
      console.error('');
      console.error('✗ Animation does not expose window.__seek (or not ready).');
      console.error('  Seek rendering only supports animations driven by the Stage clock (<Stage> from assets/animations.jsx');
      console.error('  or <NarrationStage> from narration_stage.jsx). Pure CSS @keyframes / Lottie /');
      console.error('  hand-written non-Stage animations should use render-video.js instead.');
      console.error('');
    }
    await browser.close();
  ...
  // PNG sequence → MP4. No trim (no black frames to begin with), both input and output frame rate set to FPS.
