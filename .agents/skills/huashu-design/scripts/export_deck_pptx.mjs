 ...
 *   - Text is real text boxes, double-click editable in PPT
 *   - Body size 960pt × 540pt (LAYOUT_WIDE, 13.333″ × 7.5″)
 *
 * ⚠️ HTML must satisfy 4 hard constraints (see references/editable-pptx.md):
 *   1. Text must be in <p>/<h1>-<h6> (div cannot contain text directly)
 *   2. No CSS gradients
 *   3. <p>/<h*> must not have background/border/shadow (put on outer div)
 *   4. div must not use background-image (use <img>)
 *
 * Visually-driven HTML almost never passes — you must write HTML from the first line with these constraints in mind.
 * For scenarios prioritizing visual freedom (animation, web components, CSS gradients, complex SVG),
...
function parseArgs() {
  ...
  if (!args.slides || !args.out) {
    ...
    console.error('   For scenarios prioritizing visual freedom, use export_deck_pdf.mjs to export PDF.');
    process.exit(1);
  }
  return args;
}
...
async function main() {
  ...
  if (errors.length) {
    console.error(`\n⚠️ ${errors.length} slide(s) failed conversion. Common cause: HTML does not meet the 4 hard constraints.`);
    console.error(`  See references/editable-pptx.md for "Common Errors Quick Reference".`);
    if (errors.length === files.length) {
      console.error(`✗ All failed, not generating PPTX.`);
      process.exit(1);
    }
  }
