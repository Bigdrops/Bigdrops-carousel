...
> Inspiration: Claude Design website hero video + Apple product page "gallery wall" display
> Practical reference: huashu-design released hero v5
> Applicable scenarios: **Product launch hero animation, skill capability demo, portfolio showcase** — any scenario where multiple high-quality outputs need to be displayed simultaneously while guiding the audience's attention
...
**Suitable**:
- 10+ real outputs to display on the same screen (PPT, App, web pages, infographics)
- Audience is professional (developers, designers, product managers), sensitive to "quality feel"
- Want to convey a vibe that is "restrained, exhibition-like, premium, spatial"

**Not suitable**:
- Single product focus (use the frontend-design product hero template)
- Emotional/story-driven animation (use the timeline narrative template)
- Small screen / portrait orientation (tilted perspective blurs on small canvases)
...
**Key Principles**:
1. **Never use pure black backgrounds**. Black backgrounds make work look like cinema, not "adoptable work results"
2. **Terracotta orange is the only hue accent**, everything else is grayscale + white
3. **Triple font stack** (serif-EN + serif-CN + sans + mono) creates a "publication" rather than "internet product" feel

---

## Core Layout Patterns

### 1. Floating Cards (Basic Unit of the Style)
...
.gallery-card {
  ...
  box-shadow:
    ...
    0 6px 18px -6px rgba(29, 29, 31, 0.06);      /* Second near-light layer, creating float feel */
...
**Anti-pattern**: Don't tile edge-to-edge (no padding, no border, no shadow) — that's infographic density, not an exhibition.
...
.gallery-canvas {
  ...
  height: 2520px;                        /* Leave space for panning */
  transform-origin: center center;
  transform: perspective(2400px)
             rotateX(14deg)              /* Tilt backward */
             rotateY(-10deg)             /* Turn left */
             rotateZ(-2deg);             /* Slight tilt, break excessive neatness */
  display: grid;
...
- rotateX: 10-15deg (more looks like a VIP party backdrop)
- rotateY: ±8-12deg (left-right symmetry)
- rotateZ: ±2-3deg (human touch — "not machine-placed")
- perspective: 2000-2800px (below 2000 creates fisheye, above 3000 approaches orthographic)

### 3. 2×2 Four Corner Convergence (Optional Scene)
...
4 elements slide in from viewport corners while scaling 0.85→1.0, using ease-out. Suitable for "showing multi-directional choices" as an opener.
...
### Mode B · Selected Enlarge + Others Slide Out (0.8s)

The selected card enlarges 1.0→1.28, other cards fade out + blur + drift back to the four corners:
...
**Key**: Unselected cards must blur, not just fade. Blur simulates depth of field, visually "pushing forward" the selected card.

### Mode C · Ripple Expansion (1.7s)

From center outward, each card fades in sequentially by distance delay, scaling from 1.25x to 0.94x ("lens pull-back"):
...
Uses sine wave + linear drift combination to avoid the "start and end" loop feeling of marquee:
...
- Linear drift `5-8 px/s` (slower than the audience blinking)
- Amplitude `120-220 px` (large enough to feel, small enough not to disorient)

### Mode E · Focus Overlay (Focus Switching)

**Key design**: The focus overlay is a **flat element** (not tilted), floating above the tilted canvas. The selected slide scales from its tile position (~400×225) to screen center (960×540). The background canvas doesn't change tilt but **dims to 45%**:
...
// Background cards darken but remain visible (key! no 100% mask)
card.style.opacity = entryOp * (1 - 0.55 * focusIntensity);   // 1 → 0.45
card.style.filter = `brightness(${1 - 0.3 * focusIntensity})`;
```

**Clarity Rule**:
- Focus overlay's `<img>` must `src` directly to the original image, **do not reuse compressed gallery thumbnails**
- Preload all original images into `new Image()[]` array in advance
- Overlay's own `width/height` calculated per frame, browser resamples the original every frame
...
- Can jump to any moment `window.__setTime(12.3)` instantly (convenient for Playwright frame-by-frame capture)
- Loop is naturally seamless (t mod DURATION)
- Debug: can freeze any frame

---

## Quality Details (Easy to Miss but Critical)

### 1. SVG Noise Texture

Light backgrounds' biggest fear is being "too flat." Overlay an extremely subtle fractalNoise:
...
Looks the same — you only notice when it's gone.
...
Only appears during gallery wall scenes, fades in/out. Like a museum label.
...
.brand-wordmark {
  ...
  font-size: 148px;
  font-weight: 700;
  letter-spacing: -0.045em;   /* Negative tracking is key, compacts letters into a logo */
}
.brand-wordmark .accent {
  color: var(--accent);
  font-weight: 500;           /* Accent character slightly thinner, visual contrast */
}
```

`letter-spacing: -0.045em` is the standard practice for Apple product page large text.
...
| Tilt feels cheap | Only used rotateY, no rotateZ | Add ±2-3deg rotateZ to break neatness |
| Pan feels "stuttery" | Used setTimeout or CSS keyframes loop | Use rAF + sin/cos continuous functions |
| Focus text unreadable | Reused low-res gallery tile images | Independent overlay + direct original src |
| Background too empty | Solid color `#F5F5F7` | Overlay SVG fractalNoise 0.5 opacity |
| Fonts feel too "internet" | Only Inter | Add Serif (CN + EN) + mono triple stack |

---

## References

- Full implementation sample: hero-animation-v5.html (author's local sample, not distributed with repo)
- Original inspiration: claude.ai/design hero video
- Reference aesthetics: Apple product pages, Dribbble shot collections

When encountering animation needs for "displaying multiple high-quality outputs," copy the skeleton directly from this file, swap content + adjust timing.
