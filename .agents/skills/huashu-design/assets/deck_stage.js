 ...
 * - Auto-adds data-screen-label to each slide
 *
 * Usage:
 *   <deck-stage>
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Defense: if the script is placed in <head> (rather than after </deck-stage>),
      // the parser may not have processed child <section> elements yet,
      // and querySelectorAll would return empty.
      // Defer to the next event loop to ensure all children are parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document is still parsing, wait for DOMContentLoaded to get all sections at once
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document is already parsed (script at bottom or defer), collect next frame
